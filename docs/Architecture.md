...
ドメインから永続化や外部 API を直接呼ばない。`application` 層が必要とする interface を定義し、実装を `postgres` と `github` に分ける。
...
├── application/
│   ├── get_ranking.go
│   ├── get_my_contributions.go
│   ├── get_language_detail.go
│   └── sync_github_activity.go
├── domain/
│   ├── faction.go
│   ├── season.go
│   ├── contribution.go
...
ローカルの PostgreSQL は Docker Compose で起動する。アプリケーションはホスト側で起動し、ホットリロードやデバッガを使いやすくする。
