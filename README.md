...
ローカル開発用の PostgreSQL は Docker Compose で起動します。初回は root の `.env.example` をコピーしてから起動してください。
...
API のローカル起動設定は `services/api/.env.example` をコピーして作ります。
...
- GITHUB_REDIRECT_URL: ローカルでは `http://localhost:5173/api/auth/github/callback`
- AUTH_COOKIE_SECRET: OAuth state cookie の署名 secret
- AUTH_COOKIE_SECURE: 本番 HTTPS では `true`
- FRONTEND_URL: ログイン完了後の遷移先 URL

アプリケーションからは `services/api/.env` の `DATABASE_URL` を使い、GORM 経由で接続します。別コンテナから接続する場合は host を `postgres` にしてください。GitHub OAuth の state は署名付き cookie、User と session は PostgreSQL に保存します。
